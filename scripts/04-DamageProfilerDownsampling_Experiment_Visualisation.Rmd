...
This notebook generates the visualisation of experiment on downscaling mapped-read BAM files to low-numbers of reads (typical found in dietary detection) on damageprofiles. Please see the `04-DamageProfilerDownsampling_Experiment_Processing.md` for mapping, downsampling and DamageProfile generation.
...
```{r}
load("../results/04-DamageProfilerDownsampling_Experiment_Files/damage_original_damageprofiles.robj")
```
...
```{r}
load("../results/04-DamageProfilerDownsampling_Experiment_Files/damage_downscaling_damageprofiles.robj")
```
